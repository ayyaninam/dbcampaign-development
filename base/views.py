...
def services_page(request):
    ...
    h = request.GET.get('h') if request.GET.get('h') != None else ''
    services = ""
    hearted = request.session.get('hearted')
    if hearted:
        all_ids = request.session['hearted'].split(" ")
    ...
    if h == "y":
        if hearted:
            services = Service.objects.filter(id__in=hearted)
    else:
...
def service_detail_page(request, title_str):
    service = Service.objects.get(title__exact=title_str)
    related_services = Service.objects.all()[:3]
    hearted = request.session.get('hearted')
    if hearted:
        all_ids = request.session['hearted'].split(" ")
